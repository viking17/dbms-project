...
<html lang="en">
  ...
  <body>
    ...
    <div class="jumbotron">
      <div class="container">
        <h1 class="display-3">Return Receipt</h1>
...
mysql_select_db("test", mysql_connect("127.0.0.1", "root", "password"));
 ...
 echo "<th>Name</th><th>USN</th><th>SEM</th><th>DEPT</th><th>Book_id</th><th>Book_Returned</th><th>Author</th><th>Quantity</th><th>Dept</th><th>Return_date</th>";  
 echo"<tr><td>".$row1['name']."</td><td>".$row1['usn']."</td><td>".$row1['sem']."</td><td>".$row1['dept']."</td><td>".$row['book_id']."</td><td>".$row['name']."</td><td>".$row['author']."</td><td>".$row['no_copies']."</td><td>". $row['dept']."</td><td>".$row1['o_date']."</td></tr>";
   ...
   $sql = mysql_query("DELETE FROM students WHERE usn = '$_POST[usn]'") ;
}
?>
